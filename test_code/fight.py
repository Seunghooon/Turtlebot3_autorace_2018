...
def linetrace(data):
	global angular, stage, delay, T_finish, angular_prev, timer, jucha_start, count, obstacle
	...
	if i==2:
		...
		angular = -(L_degree + R_degree)*0.009
	...
	if stage==10 and i>=1 and count>20:
		...
		if T_finish==1 and count>40:

			stage=100

		if jucha_start==1 and count>40:
	...
	if stage==111:
		...
		for c in blue_contours:
			...
			if w>30 and w<160 and h>30 and h<90 and end==160 :
				blue_x.append(x)
				blue_y.append(y)
		...
		if len(blue_x)>0 and timer>140:
		
			turtlemove(0,0)
			rospy.sleep(rospy.Duration(0.2))

			#turtlemove(0.1,0)
			rospy.sleep(rospy.Duration(1.35))
	...
	if stage ==100 or stage==10010 or stage==10011 or stage==1111 and parking_count==0:
		...
		if stage==10010:
			...
			for c in blue_contours:
				...
				if w>5 and w<25 and h>5 and h<25 and y>150 and x>300 :
					blue_x.append(x)
					blue_y.append(y)
			...
			if len(blue_x)==1 and delay>70 and nn>0 and nn<10:
		...
		if stage==10011:
			...
			for c in blue_contours:
				...
				if w>5 and w<25 and h>5 and h<20 and x <25 and y>150 :
					blue_x.append(x)
					blue_y.append(y)
					cv2.drawContours(frame, [c], -1, (255, 0, 0), 3)

			if len(blue_x)==1 and delay>70 and n>0 and n<10:
...
def traffic_sign(data):
	global stage, jucha_finish,jucha_exit,count, parking_count;
	...
	if T_finish==0:
		...
		for c in center_blue_contours:
			...
			if w>10 and w<110 and h<60 and x<250 and end<280 and len(approx)!=6:
				#print('x',x,'y',y,'w',w,'h',h,'len(approx',len(approx))
	...
	if stage == 100: #stage select
		...
		elif len(center_blue_x)==2 and T_finish==0  and nnnn>=3 and abs(center_blue_y[0]-center_blue_y[1])>50 and center_blue_y[0]*center_blue_y[1]==0:
			...
			if len(contours)==2:
				...
				if dot_line>=2 and jucha_finish==0:
					...
					if parking>0:
						...
						rospy.sleep(rospy.Duration(0.3))
						turtlemove(0.15,0)
						rospy.sleep(rospy.Duration(1.55))
						turtlemove(0,0)
						rospy.sleep(rospy.Duration(1))
						turtlemove(-0.15,0)
						rospy.sleep(rospy.Duration(1.55))
...
def chadan_dist(data):
	...
	if dist_chadan<=30 and dist_chadan>0 and stage==100 and nnnn==0:
		stage=2
		print("chadan!")
		turtlemove(0,0)
...
def checking_distance(lidar):
	global n, nn, nnn, nnnn, parking, stage,obstacle_finish, right, all_around
	
	d=lidar.ranges[0:90]
	dd=lidar.ranges[270:360]
	dddd=lidar.ranges[0:20]+lidar.ranges[345:360]
	ddddd=lidar.ranges[10:90]
	...
	if stage==100 and all_around>35 and all_around<55 and obstacle_finish==0 and T_finish==1 and n>5 and nnnn>=10 and nnnn<25 and dist_chadan<35 and dist_chadan>20:
